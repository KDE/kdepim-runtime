...
void QwtDoubleRange::setNewValue(double x, int align)
{
    ...
    if (x < vmin) {
        if ((d_periodic) && (vmin != vmax)) {
           d_xOverflow = ceil( (vmin - x) / (vmax - vmin ) ) 
              * (vmax - vmin);
           d_value = x + d_xOverflow;
        } else {
           d_value = vmin;
           d_xOverflow = 0;
        }
        ...
        if ((d_periodic) && (vmin != vmax)) {
           d_xOverflow = - ceil( ( x - vmax) / (vmax - vmin )) 
              * (vmax - vmin);
           d_value = x + d_xOverflow;
        } else {
           d_value = vmax;
    ...
    int diff = int( d_value - d_prevValue );
    if ( abs( diff ) > ( vmax - vmin ) / 2 ) {
      d_overflow = diff / abs( diff ) * ( vmax - vmin );
    } else {
      d_overflow = 0;
    }
...
double QwtDoubleRange::overflow() const
{
    return d_overflow;
}

...
void QwtAbstractSlider::emitValueDelta()
{
  if ( prevValue() == value() && overflow() == 0 ) return;

  double delta = value() - prevValue();

#if 0
  qDebug() << "ORIG DELTA: " << delta;
#endif

  delta -= overflow();

#if 0
  qDebug() << "  EMIT PREV" << prevValue() << "VALUE" << value() << "OVERFLOW"
    << overflow() << "DELTA" << delta;
#endif
...
#include "qwt_abstract_slider.moc"

...
public:
  ...
  static KCalCore::Journal* xmlToJournal( const QString& xml );

  /// Use this to get an xml string describing this journal entry
  static QString journalToXML( KCalCore::Journal* );

  /// Create a note object and
  explicit Note( KCalCore::Journal* journal = 0 );
  virtual ~Note();

  void saveTo( KCalCore::Journal* journal );
...
protected:
  ...
  void setFields( const KCalCore::Journal* );

  // Save all known fields into this ical incidence
  void saveTo( KCalCore::Incidence* ) const;

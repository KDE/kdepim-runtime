...
QString etesync_auth_get_token(const EteSync *etesync, const QString &username, const QString &password);

qint32 etesync_auth_invalidate_token(const EteSync *etesync, const QString &token);

EteSyncCollectionInfoPtr etesync_collection_info_new(const QString &col_type, const QString &display_name, const QString &description, qint32 color);

QString etesync_crypto_derive_key(const EteSync *etesync, const QString &salt, const QString &password);

EteSyncEntryPtr etesync_entry_from_sync_entry(const EteSyncCryptoManager *crypto_manager, const EteSyncSyncEntry *sync_entry, const QString &prev_uid);

EteSyncSyncEntryPtr etesync_entry_get_sync_entry(const EteSyncEntry *entry, const EteSyncCryptoManager *crypto_manager, const QString &prev_uid);

qint32 etesync_entry_manager_create(const EteSyncEntryManager *entry_manager, const EteSyncEntry *const *entries, const QString &prev_uid);

std::pair<std::vector<EteSyncEntryPtr>, bool> etesync_entry_manager_list(const EteSyncEntryManager *entry_manager, const QString &prev_uid, uintptr_t limit);

EteSyncEntryManagerPtr etesync_entry_manager_new(const EteSync *etesync, const QString &journal_uid);

EteSyncCryptoManagerPtr etesync_journal_get_crypto_manager(const EteSyncJournal *journal, const QString &key, const EteSyncAsymmetricKeyPair *keypair);

EteSyncJournalPtr etesync_journal_manager_fetch(const EteSyncJournalManager *journal_manager, const QString &journal_uid);
...
EteSyncSyncEntryPtr etesync_sync_entry_new(const QString &action, const QString &content);

EteSyncCryptoManagerPtr etesync_user_info_get_crypto_manager(const EteSyncUserInfo *user_info, const QString &key);

EteSyncUserInfoPtr etesync_user_info_manager_fetch(const EteSyncUserInfoManager *user_info_manager, const QString &owner);

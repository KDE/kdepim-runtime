...
{
    qCDebug(KOLABRESOURCE_LOG) << "setting authcombo: " << authenticationModeString(authtype);
    int index = authCombo->findData(authtype);
    if (index == -1) {
        qCWarning(KOLABRESOURCE_LOG) << "desired authmode not in the combo";
    }
    qCDebug(KOLABRESOURCE_LOG) << "found corresponding index: " << index << "with data" << authenticationModeString((MailTransport::Transport::EnumAuthenticationType::type) authCombo->itemData(index).toInt());
    ...
    qCDebug(KOLABRESOURCE_LOG) << "selected auth mode:" << authenticationModeString(t);
    Q_ASSERT(t == authtype);
}
...
static MailTransport::Transport::EnumAuthenticationType::type getCurrentAuthMode(QComboBox *authCombo)
{
    ...
    qCDebug(KOLABRESOURCE_LOG) << "current auth mode: " << authenticationModeString(authtype);
    return authtype;
}

static void addAuthenticationItem(QComboBox *authCombo, MailTransport::Transport::EnumAuthenticationType::type authtype)
{
    qCDebug(KOLABRESOURCE_LOG) << "adding auth item " << authenticationModeString(authtype);
...
void SetupServer::slotEncryptionRadioChanged()
{
    ...
    case KIMAP::LoginJob::AnySslVersion:
        ...
        break;
    default:
        qCCritical(KOLABRESOURCE_LOG) << "Shouldn't happen";
    }
}
...
void SetupServer::applySettings()
{
    ...
    qCDebug(KOLABRESOURCE_LOG) << "saving IMAP auth mode: " << authenticationModeString(authtype);
    ...
    qCDebug(KOLABRESOURCE_LOG) << "wrote" << m_ui->imapServer->text() << m_ui->userName->text() << m_ui->safeImapGroup->checkedId();
...
void SetupServer::readSettings()
{
    ...
    qCDebug(KOLABRESOURCE_LOG) << m_parentResource->settings()->imapPort();
    qCDebug(KOLABRESOURCE_LOG) <<  m_parentResource->settings()->safety();
    ...
    qCDebug(KOLABRESOURCE_LOG) << "read IMAP auth mode: " << authenticationModeString((MailTransport::Transport::EnumAuthenticationType::type) i);
...
{
    qCDebug(KOLABRESOURCE_LOG) << m_ui->imapServer->text();
    ...
    qCDebug(KOLABRESOURCE_LOG) << "server: " << server << "port: " << port;
...
{
    qCDebug(KOLABRESOURCE_LOG) << testResult;
...
{
    if (m_serverTest == 0) {
        qCDebug(KOLABRESOURCE_LOG) << "serverTest null";
    ...
    switch (m_ui->safeImapGroup->checkedId()) {
    case KIMAP::LoginJob::Unencrypted :
        qCDebug(KOLABRESOURCE_LOG) << "safeImapGroup: unencrypted";
        protocols = m_serverTest->normalProtocols();
        break;
    case KIMAP::LoginJob::AnySslVersion:
        protocols = m_serverTest->secureProtocols();
        qCDebug(KOLABRESOURCE_LOG) << "safeImapGroup: SSL";
        break;
    case KIMAP::LoginJob::TlsV1:
        protocols = m_serverTest->tlsProtocols();
        qCDebug(KOLABRESOURCE_LOG) << "safeImapGroup: starttls";
        break;
    default:
        qCCritical(KOLABRESOURCE_LOG) << "Shouldn't happen";
    }
    ...
    if (protocols.size() > 0) {
        ...
        qCDebug(KOLABRESOURCE_LOG) << "no authmodes found";
    }
}

void SetupServer::slotManageSubscriptions()
{
    qCDebug(KOLABRESOURCE_LOG) << "manage subscripts";
    ImapAccount account;

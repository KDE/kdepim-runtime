...
void ChangeItemTask::onMessagesReceived(const QMap<qint64, KIMAP::Message> &messages)
{
    ...
    scope.mode = KIMAP::FetchJob::FetchScope::Flags;
    bool ok;
    m_messageFetched = resourceState()->messageHelper()->createItemFromMessage(msg->message, msg->uid, msg->size, msg->attributes, msg->flags, scope, ok);
    if (!ok) {
...
#include "moc_changeitemtask.cpp"

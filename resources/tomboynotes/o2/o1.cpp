                                 ...
                                 QNetworkAccessManager::Operation operation)
{
    ...
    if (signatureMethod() == QLatin1StringView(O2_SIGNATURE_TYPE_HMAC_SHA1)) {
        signature = sign(headers, signingParameters, req.url(), operation, clientSecret(), tokenSecret());
    } else if (signatureMethod() == QLatin1StringView(O2_SIGNATURE_TYPE_PLAINTEXT)) {
...
void O1::link()
{
    ...
    request.setHeader(QNetworkRequest::ContentTypeHeader, QLatin1StringView(O2_MIME_TYPE_XFORM));
...
void O1::onTokenRequestFinished()
{
    ...
    requestToken_ = response.value(QLatin1StringView(O2_OAUTH_TOKEN), QString());
    requestTokenSecret_ = response.value(QLatin1StringView(O2_OAUTH_TOKEN_SECRET), QString());
    setToken(requestToken_);
    setTokenSecret(requestTokenSecret_);

    // Checking for "oauth_callback_confirmed" is present and set to true
    QString oAuthCbConfirmed = response.value(QLatin1StringView(O2_OAUTH_CALLBACK_CONFIRMED), QStringLiteral("false"));
    if (requestToken_.isEmpty() || requestTokenSecret_.isEmpty() || (oAuthCbConfirmed == QLatin1StringView("false"))) {
...
#else
    QUrlQuery query(url);
    query.addQueryItem(QLatin1StringView(O2_OAUTH_TOKEN), requestToken_);
    query.addQueryItem(QLatin1StringView(O2_OAUTH_CALLBACK), QString::fromLatin1(callbackUrl().arg(replyServer_->serverPort()).toLatin1()));
    url.setQuery(query);
#endif
...
void O1::onVerificationReceived(const QMultiMap<QString, QString> &params)
{
    ...
    Q_EMIT closeBrowser();
    verifier_ = params.value(QLatin1StringView(O2_OAUTH_VERFIER), QString());
    if (params.value(QLatin1StringView(O2_OAUTH_TOKEN)) == requestToken_) {
...
void O1::onTokenExchangeFinished()
{
    ...
    QByteArray data = reply->readAll();
    QMap<QString, QString> response = parseResponse(data);
    if (response.contains(QLatin1StringView(O2_OAUTH_TOKEN)) && response.contains(QLatin1StringView(O2_OAUTH_TOKEN_SECRET))) {
        setToken(response.take(QLatin1StringView(O2_OAUTH_TOKEN)));
        setTokenSecret(response.take(QLatin1StringView(O2_OAUTH_TOKEN_SECRET)));

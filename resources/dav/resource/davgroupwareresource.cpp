...
void DavGroupwareResource::collectionAdded(const Akonadi::Collection &collection, const Akonadi::Collection &parent)
{
    ...
    fetchJob->setProperty("parentName", QVariant::fromValue(parent.name()));
    fetchJob->setProperty("davUrl", QVariant::fromValue(*davUrlIt));

    connect(fetchJob, &KDAV::DavPrincipalHomeSetsFetchJob::result, this, [this](KJob *job) {
        const auto collection = job->property("collection").value<Collection>();
        const auto parentName = job->property("parentName").value<QString>();
